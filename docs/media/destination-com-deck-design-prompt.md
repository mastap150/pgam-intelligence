...
- **"Turn this into a .pptx"** — already built: `destination-com-media-kit.pptx`,
  generated by `build-media-kit-pptx.js` in this directory (`npm install pptxgenjs
  && node build-media-kit-pptx.js`). Eleven slides, navy-dominant, safe-list fonts
  (Century Schoolbook + Calibri), speaker notes on every slide. Edit the generator
  and re-run rather than editing the .pptx by hand.
