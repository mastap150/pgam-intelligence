...
## TB-specific implementation checklist

Before any agent ships on TB, fill in these blanks:

- [ ] What's TB's equivalent of `set_demand_floor`? (the canonical write API)
- [ ] What's TB's equivalent of `biddingpreferences[].value[]`? (per-pub demand entry)
- [ ] What's TB's equivalent of `demand_id`? (unit of floor write)
- [ ] What's TB's equivalent of `publisher_id`? (entity that runs demands)
- [ ] What contract minimums exist on TB? (analog of 9 Dots $1.70)
- [ ] Does TB have multi-entity floor writes (where one write affects N entities)?
- [ ] Does TB have a verifiable read-after-write API? (for `verify=True`)
- [ ] What's TB's revenue/funnel data source? (analog of `data/hourly_pub_demand.json.gz`)
- [ ] What's TB's deploy target? (Render/Railway/etc — same plan-size lessons apply)
- [ ] What's TB's Slack webhook? (separate from LL's? or shared?)
- [ ] Does TB have its own LLM advisory output today? If yes, wrap it in advisory_verifier
- [ ] What env var pattern does TB use? (mirror `PGAM_*_ENABLED` flags for safe rollout)

Once those are answered, port `core/floor_ledger.py` and the write-path
clamp first. Then the defensive agents. Then everything else in the
